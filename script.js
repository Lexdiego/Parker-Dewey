...
// Configure this at runtime only; never commit API keys to client-side code.
const OPENROUTER_API_KEY = window.localStorage.getItem("openrouter_api_key");
...
async function getAIRecommendations(
  ...
  const localRecommendations = getRecommendations(
    hvacType,
    buildingAge,
    buildingSize,
    energyBill,
...
Provide practical, measurable recommendations that would help improve this building's energy efficiency. Each recommendation should be specific and could potentially reduce energy costs or improve sustainability.`;
    ...
    // Combine with local recommendations for more comprehensive list
    return [...localRecommendations, ...aiRecommendations];
  } catch (error) {
    console.error("Error fetching AI recommendations:", error);
    // Fallback to local recommendations if API fails
    return getRecommendations(hvacType, buildingAge, buildingSize, energyBill);
  }
}
